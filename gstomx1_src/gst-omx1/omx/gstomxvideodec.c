    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_ENABLE_ERROR_CHECK:
      ...
      break;
    case PROP_USE_LOW_OUT_BUFFER:
      self->use_low_buf = g_value_get_boolean (value);
      break;
    case PROP_DISABLE_DVFS:
      self->disable_dvfs = g_value_get_boolean (value);
      break;
#endif
    default:
    ...
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_ENABLE_ERROR_CHECK:
      ...
      break;
    case PROP_USE_LOW_OUT_BUFFER:
      g_value_set_boolean (value, self->use_low_buf);
      break;
    case PROP_DISABLE_DVFS:
      g_value_set_boolean (value, self->disable_dvfs);
      break;
#endif
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
...
gst_omx_video_dec_class_init (GstOMXVideoDecClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_ENABLE_ERROR_CHECK,
      g_param_spec_boolean ("enable-error-check",
          ...
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_USE_LOW_OUT_BUFFER,
      g_param_spec_boolean ("enable-low-outbuffer",
          "Enable low out hw buffer allocation",
          "Set to enable low output hardware buffer allocation",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_DISABLE_DVFS,
      g_param_spec_boolean ("disable-dvfs",
          "Disable dvfs",
          "Set to disable dvfs",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
...
gst_omx_video_dec_init (GstOMXVideoDec * self)
{
  ...
  self->use_low_buf = FALSE;
  self->disable_dvfs = FALSE;
#endif
...
gst_omx_video_dec_open (GstVideoDecoder * decoder)
{
  ...
  if (self->use_low_buf) {
    NVX_PARAM_USELOWBUFFER param;
    OMX_INDEXTYPE eIndex;
    OMX_ERRORTYPE err = OMX_ErrorNone;

    err = gst_omx_component_get_index (self->dec,
        (char *) NVX_INDEX_PARAM_USELOWBUFFER, &eIndex);

    if (err == OMX_ErrorNone) {
      GST_OMX_INIT_STRUCT (&param);
      param.nPortIndex = self->dec_out_port->index;
      param.bUseLowBuffer = OMX_TRUE;
      err = gst_omx_component_set_parameter (self->dec, eIndex, &param);
    } else {
      g_print ("Coudn't get extension index for %s\n",
        (char *) NVX_INDEX_PARAM_USELOWBUFFER);
    }

    if (err != OMX_ErrorNone) {
      g_print ("Couldn't use low buffer\n");
    }
  }

  if (self->disable_dvfs) {
    OMX_CONFIG_BOOLEANTYPE disable_dvfs;
    OMX_INDEXTYPE eIndex;
    OMX_ERRORTYPE err = OMX_ErrorNone;

    err = gst_omx_component_get_index (self->dec,
        (char *) NVX_INDEX_PARAM_VIDEO_DISABLE_DVFS, &eIndex);

    if (err == OMX_ErrorNone) {
      GST_OMX_INIT_STRUCT (&disable_dvfs);
      disable_dvfs.bEnabled = OMX_TRUE;
      err = gst_omx_component_set_parameter (self->dec, eIndex, &disable_dvfs);
    } else {
      g_print ("Coudn't get extension index for %s\n",
        (char *) NVX_INDEX_PARAM_VIDEO_DISABLE_DVFS);
    }

    if (err != OMX_ErrorNone) {
      g_print ("Couldn't use disable dvfs\n");
    }
  }
#endif
...
#undef OMX_IndexParamBrcmVideoEGLRenderDiscardMode
...
#if defined (USE_OMX_TARGET_TEGRA) && defined (HAVE_GST_EGL)
  {
    ...
    if ((!self->use_low_buf) && (!self->cpu_dec_buf))
...
gst_omx_video_dec_negotiate (GstOMXVideoDec * self)
{
  ...
  /* NOTE: Extra Pitch Linear buffer allocation for decoder output
     by omx-il is avoided using check for use_low_buf flag. Hence,
     decoder raw-yuv buffer output is only supported for usecases
     where low output memory allocation is not a requirement and
     use_low_buf flag is disabled. */
  gst_omx_video_dec_set_cpu_buf (self);
