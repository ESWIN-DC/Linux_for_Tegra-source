...
gst_omx_h264_enc_init (GstOMXH264Enc * self)
{
  ...
  self->cabac_entropy_coding = FALSE;
    ...
    if (err == OMX_ErrorNone) {
      ...
      if (profile_string) {
        if (g_str_equal (profile_string, "baseline") ||
          g_str_equal (profile_string, "constrained-baseline")) {
...
gst_omx_set_avc_encoder_property (GstOMXVideoEnc * enc)
{
  ...
  if (eError == OMX_ErrorNone) {
    ...
    if ((oH264Type.eProfile == OMX_VIDEO_AVCProfileBaseline) &&
        (self->cabac_entropy_coding != FALSE))
    {
        GST_WARNING_OBJECT (self,
            "CABAC is not supported for Baseline profile");
        oH264Type.bEntropyCodingCABAC = FALSE;
    }
    else
        oH264Type.bEntropyCodingCABAC = self->cabac_entropy_coding;
