...
GST_STATIC_PAD_TEMPLATE ("src",
    ...
    (GST_CAPS_FEATURE_MEMORY_RMSURFACE, "{ " "I420, GRAY8 }") ";"
    GST_VIDEO_CAPS_MAKE
...
#if defined (USE_TARGET_TEGRA)
  g_object_class_install_property (gobject_class, PROP_IS_DEEPSTREAM,
          g_param_spec_boolean ("DeepStream",
              "Enable JPEG Decode in DS pipeline",
              "Enable JPEG Decode in DS pipeline, legacy decode enabled by default",
              FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
              GST_PARAM_MUTABLE_READY));
#endif
#if defined (USE_TARGET_GPU)
...
gst_jpeg_dec_open (GstVideoDecoder * bdec)
{
  ...
  {
      dec->bis_deepstream =  dec->cinfo.is_deepstream = TRUE;
  }
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PERF_MEASUREMENT:
      ...
      break;
#if defined (USE_TARGET_TEGRA)
    case PROP_IS_DEEPSTREAM:
      dec->bis_deepstream = g_value_get_boolean (value);
      break;
#endif
#if defined (USE_TARGET_GPU)
...
#if defined (USE_TARGET_TEGRA)
    case PROP_IS_DEEPSTREAM:
      g_value_set_boolean (value, dec->bis_deepstream);
      break;
#endif
#if defined (USE_TARGET_GPU)
    case PROP_GPU_DEVICE_ID:
