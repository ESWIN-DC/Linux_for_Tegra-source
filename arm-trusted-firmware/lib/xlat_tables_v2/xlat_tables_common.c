...
static int32_t xlat_tables_mapped_regions[MAX_XLAT_TABLES];
...
static xlat_ctx_t tf_xlat_ctx = {
	...
	.base_table_entries = (uint32_t)NUM_BASE_LEVEL_ENTRIES,
...
int32_t mmap_add_dynamic_region(uint64_t base_pa,
			    uintptr_t base_va, size_t size, uint32_t attr)
{
	mmap_region_t mm = {
		.base_va = base_va,
		.base_pa = base_pa,
...
void init_xlat_tables(void)
{
	...
	assert(tf_xlat_ctx.initialized == 0U);
	...
	assert(tf_xlat_ctx.max_va <= (PLAT_VIRT_ADDR_SPACE_SIZE - 1U));
	assert(tf_xlat_ctx.max_pa <= (PLAT_PHY_ADDR_SPACE_SIZE - 1U));
