...
function main() {
  if [ ! -d patches ]; then
    echo "OpenSSL patch directory patches/ not found"
  fi
...
function generate_build_config_headers() {
  chmod +x ./Configure
  PERL=/usr/bin/perl run_verbose ./Configure $CONFIGURE_ARGS
  PERL=/usr/bin/perl run_verbose make include/openssl/opensslconf.h
  PERL=/usr/bin/perl run_verbose make include/crypto/bn_conf.h
  PERL=/usr/bin/perl run_verbose make include/crypto/dso_conf.h
  PERL=/usr/bin/perl run_verbose make crypto/buildinf.h
...
function generate_build_config_mk() {
  chmod +x ./Configure
  generate_build_config_headers trusty

  echo "Generating build-config-trusty.mk"
  (
    print_autogenerated_header

    echo "openssl_cflags_trusty := \\"
...
function generate_config_mk() {
  declare -r output="$1"
  declare -r prefix="$2"
  declare -r all_archs="arm"
  declare -r variant_archs=""
